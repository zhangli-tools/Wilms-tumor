...
legend=TRUE){
    ...
    if(class(sourceOmics)[1]=="SummarizedExperiment"){
    ...
    for(i in seq_len(length(targetOmicsList))){
      if(class(targetOmicsList[[i]][1])=="SummarizedExperiment"){
...
calculateChromLength(chromLength,chrome_targetOmics,genelocate_targetOmics)
    ...
    if(length(targetOmicsList)==1){
      ...
      pdf(outputfile,height = 10,width = 8*length(targetOmicsList))
